...
class RikaFirenet extends utils.Adapter {
  ...
  async onReady() {
    ...
    if (
      !this.config.mystoveid ||
      !this.config.myuser ||
      !this.config.mypassword
    ) {
      this.log.error(
    ...
    this.isReady = true;
    this.sessionId =
      (await this.webLogin(this.config.myuser, this.config.mypassword)) || "";
  ...
  async setstoveValues(controlname, controlvalue) {
    ...
    try {
      const response = await client.get(
        `${baseUrl}/api/client/${stoveID}/status`,
      );
      ...
      if (response.status == 200 && response.data.indexOf(stoveID) > -1) {
        ...
        await client.post(
          `${baseUrl}/api/client/${stoveID}/controls`,
          content.controls,
        );
